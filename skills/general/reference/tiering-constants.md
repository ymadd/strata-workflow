...
- **The sonnet tier runs on the 1M-context variant (`sonnet[1m]`).** Every mode resolves its `'sonnet'` tier to `'sonnet[1m]'` at the `agent()` boundary so the cheap-bulk reasoners carry the long inputs (diffs, whole codebases, transcripts) without truncation. Implemented WITHOUT touching the pinned literals the verifiers assert: TIER-map modes append a one-line upgrade (`for (const k in TIER) if (TIER[k] === 'sonnet') TIER[k] = 'sonnet[1m]'`) after any `tierHint` mutation; named-constant modes (scale/grow/delegate/conduct/audit) wrap the model arg with a `longCtx` helper (`m === 'sonnet' ? 'sonnet[1m]' : m`; haiku/opus/fable pass through). The `TIER`/`*_MODEL` source literals stay `'sonnet'`, so both invariant suites (which grep those literals) remain green. Opus/haiku/fable are unchanged.
