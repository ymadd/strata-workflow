...
A **thin launcher**, not a separate engine. It fixes `domain = finance` and delegates to the shared Strata router and the same bundled workflows — the cap / model-tiering / gated-verify guarantees live in **one** place (`../strata-workflow/`), never duplicated here.

On activation:
1. Read the **main router** at `${CLAUDE_SKILL_DIR}/../strata-workflow/SKILL.md` and follow its activation steps, with **`domain` already fixed to `finance`** (skip its domain-classification — the domain is decided).
2. Load the finance profile at `${CLAUDE_SKILL_DIR}/../strata-workflow/reference/domains/finance.md` and merge its preset for the chosen mode into args (**caller args > profile > mode defaults**; `qualityBar`+`pitfalls` → `constraints`).
...
4. Workflows: `${CLAUDE_SKILL_DIR}/../strata-workflow/workflows/strata-<mode>.js`; references: `${CLAUDE_SKILL_DIR}/../strata-workflow/reference/`. Resolve to absolute paths before calling `Workflow`.
