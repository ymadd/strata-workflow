...
2. Load the code profile at `${CLAUDE_SKILL_DIR}/../general/reference/domains/code.md` and merge its preset for the chosen mode into args (**caller args > profile > mode defaults**; `qualityBar`+`pitfalls` → `constraints` for panel/debate/research, else prepended to the `task` text).
